...
如果你想为某个特定的实体创建应用服务并且有相应的方法：**Create，Delete，Get，GetAll**，为了更容易的实现该功能，你可以继承 **CrudAppService**(或者 **AsyncCrudAppService** 如果你想要创建async方法)类。**CrudAppService** 是一个可扩展的泛型基类，该类的泛型参数是与之相关的 **Entity和DTO** 类，并且你可以根据你的需要自定义覆盖基类的功能。
